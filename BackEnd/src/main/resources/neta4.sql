...
INSERT INTO `sys_url` VALUES (8, '/eta4/work');
INSERT INTO `sys_url` VALUES (9, '/eta4/work-add');
INSERT INTO `sys_url` VALUES (10, '/eta4/student-management');
INSERT INTO `sys_url` VALUES (11, '/eta4/teacher-management');
INSERT INTO `sys_url` VALUES (12, '/eta4/import-work');
...
CREATE TABLE `t_work`  (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT 'id',
  `workName` varchar(100) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '作业名称',
  `workTypeId` int(11) NOT NULL COMMENT '作业类型',
  ...
  PRIMARY KEY (`id`) USING BTREE,
  INDEX `fk_t_work_t_work_type_1`(`workTypeId`) USING BTREE,
  CONSTRAINT `fk_t_work_t_work_type_1` FOREIGN KEY (`workTypeId`) REFERENCES `t_work_type` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE = InnoDB AUTO_INCREMENT = 37 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '作业表' ROW_FORMAT = Dynamic;

-- ----------------------------
-- Records of t_work
-- ----------------------------
INSERT INTO `t_work` VALUES (21, '江苏省高校大学生物理及实验科技作品创新竞赛', 1, '2018-05-04 21:21:39', '2018-05-19 12:10:27');
INSERT INTO `t_work` VALUES (22, '全国大学生电子设计竞赛', 1, '2018-05-04 21:21:39', '2018-05-19 12:10:29');
INSERT INTO `t_work` VALUES (23, '全国大学生“飞思卡尔”杯智能汽车竞赛', 1, '2018-05-04 21:21:39', '2018-05-19 12:10:30');
INSERT INTO `t_work` VALUES (25, '全国大学生“用友”杯创业设计大赛', 1, '2018-05-04 21:21:39', '2018-05-19 12:10:32');
INSERT INTO `t_work` VALUES (26, '全国大学生数学建模竞赛', 1, '2018-05-04 21:21:39', '2018-05-19 12:10:33');
INSERT INTO `t_work` VALUES (27, '全国大学生基础力学实验竞赛', 1, '2018-05-04 21:21:39', '2018-05-19 12:10:35');
INSERT INTO `t_work` VALUES (28, '全国软件专业人才设计与开发大赛', 1, '2018-05-04 21:21:39', '2018-05-19 12:10:36');
INSERT INTO `t_work` VALUES (30, '全国大学生电子商务“创新、创意及创业”挑战赛', 1, '2018-05-04 21:21:39', '2018-05-19 12:10:37');
INSERT INTO `t_work` VALUES (32, '江苏省大学生职业规划大赛', 1, '2018-05-04 21:21:39', '2018-05-19 12:10:39');
INSERT INTO `t_work` VALUES (33, '“Omron杯”Sysmac自动化控制应用设计大赛', 1, '2018-05-04 21:21:39', '2018-05-19 12:10:40');
INSERT INTO `t_work` VALUES (34, '江苏省大学生电子设计竞赛', 1, '2018-05-04 21:21:39', '2018-05-19 12:10:41');
INSERT INTO `t_work` VALUES (35, '“学创杯”全国大学生创业综合模拟大赛', 1, '2018-05-04 21:21:39', '2018-05-19 12:10:42');
INSERT INTO `t_work` VALUES (36, '“蓝桥杯”全国软件和信息技术专业人才大赛', 1, '2018-05-04 21:21:39', '2018-05-19 12:10:58');
...
CREATE TABLE `t_work_type`  (
  ...
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '作业类型表' ROW_FORMAT = Dynamic;

-- ----------------------------
-- Records of t_work_type
-- ----------------------------
INSERT INTO `t_work_type` VALUES (1, '学生作业');
INSERT INTO `t_work_type` VALUES (2, '教师作业');
...
CREATE TABLE `t_teacher_link_student`  (
  ...
  `teacherId` varchar(20) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '辅导员id',
  PRIMARY KEY (`id`) USING BTREE,
  INDEX `fk_t_teacher_link_student_t_class_1`(`classId`) USING BTREE,
  INDEX `fk_t_teacher_link_student_t_teacher_1`(`teacherId`) USING BTREE,
  CONSTRAINT `fk_t_teacher_link_student_t_class_1` FOREIGN KEY (`classId`) REFERENCES `t_class` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT,
  CONSTRAINT `fk_t_teacher_link_student_t_teacher_1` FOREIGN KEY (`teacherId`) REFERENCES `t_teacher` (`teaNo`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE = InnoDB AUTO_INCREMENT = 3 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '辅导员审核学生作业关联表' ROW_FORMAT = Dynamic;

-- ----------------------------
-- Records of t_teacher_link_student
-- ----------------------------
INSERT INTO `t_teacher_link_student` VALUES (1, 382151, '100000621');
INSERT INTO `t_teacher_link_student` VALUES (2, 382152, '100000625');
...
INSERT INTO `t_role` VALUES (3, '辅导员', 'teacher');
...
-- ----------------------------
INSERT INTO `t_student` VALUES (1, '38215101', '范玉铃', 2, 382151);
INSERT INTO `t_student` VALUES (2, '38215102', '关铃娜', 2, 382152);
...
INSERT INTO `t_user` VALUES (3, '38215102', '38215102', '2018-04-25 21:22:21', '2018-07-24 12:02:04');
...
CREATE TABLE `t_user_work`  (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `workTime` date NOT NULL COMMENT '作业时间',
  `workPlace` varchar(50) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '作业名次',
  `createAt` datetime(0) NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '上传时间',
  `reviewAt` datetime(0) DEFAULT NULL COMMENT '审核时间',
  ...
  CONSTRAINT `fk_t_user_work_t_work_1` FOREIGN KEY (`workId`) REFERENCES `t_work` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `fk_t_user_work_t_review_1` FOREIGN KEY (`reviewId`) REFERENCES `t_review` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT `fk_t_user_work_t_user_1` FOREIGN KEY (`userId`) REFERENCES `t_user` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE = InnoDB AUTO_INCREMENT = 25 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '用户作业信息表' ROW_FORMAT = Dynamic;

-- ----------------------------
-- Records of t_user_work
-- ----------------------------
INSERT INTO `t_user_work` VALUES (22, '2017-12-01', '一等奖', '2018-07-08 13:59:36', NULL, '\\upload\\382151\\38215131_2018_07_08_13_59_36.jpeg', 28, 22, 1, 2);
INSERT INTO `t_user_work` VALUES (23, '2017-12-01', '一等奖', '2018-07-08 13:59:41', '2018-07-25 18:30:59', '\\upload\\382151\\38215131_2018_07_08_13_59_41.jpeg', 28, 22, 1, 2);
INSERT INTO `t_user_work` VALUES (24, '2017-12-01', '一等奖', '2018-07-08 13:59:43', NULL, '\\upload\\382151\\38215131_2018_07_08_13_59_43.jpeg', 28, 22, 1, 1);
...
INSERT INTO `t_user_role` VALUES (46, 47, 1);
INSERT INTO `t_user_role` VALUES (47, 48, 3);
INSERT INTO `t_user_role` VALUES (48, 49, 3);
...
CREATE VIEW `v_student_work` AS select `t_user_work`.`id` AS `id`,`t_user_work`.`workTime` AS `workTime`,`t_user_work`.`workPlace` AS `workPlace`,`t_user_work`.`createAt` AS `createAt`,`t_user_work`.`reviewAt` AS `reviewAt`,`t_user_work`.`imagePath` AS `imagePath`,`t_work`.`workName` AS `workName`,`t_rank`.`rankName` AS `rankName`,`t_review`.`reviewName` AS `reviewName`,`v_student_info`.`username` AS `username`,`v_student_info`.`stuNo` AS `stuNo`,`v_student_info`.`name` AS `name`,`v_student_info`.`className` AS `className`,`v_student_info`.`genderName` AS `genderName`,`v_student_info`.`gradeName` AS `gradeName`,`v_student_info`.`majorName` AS `majorName`,`t_user_work`.`userId` AS `userId`,`t_user_work`.`workId` AS `workId`,`t_user_work`.`reviewId` AS `reviewId`,`v_student_info`.`genderId` AS `genderId`,`v_student_info`.`classId` AS `classId`,`v_student_info`.`gradeId` AS `gradeId`,`v_student_info`.`majorId` AS `majorId`,`t_teacher_link_student`.`teacherId` AS `teacherId` from (((((`t_user_work` join `t_work` on((`t_user_work`.`workId` = `t_work`.`id`))) join `t_review` on((`t_user_work`.`reviewId` = `t_review`.`id`))) join `v_student_info` on((`t_user_work`.`userId` = `v_student_info`.`userId`))) join `t_teacher_link_student` on((`v_student_info`.`classId` = `t_teacher_link_student`.`classId`)));
...
-- ----------------------------
-- View structure for v_teacher_work
-- ----------------------------
DROP VIEW IF EXISTS `v_teacher_work`;
CREATE VIEW `v_teacher_work` AS select `t_user_work`.`id` AS `id`,`t_user_work`.`workTime` AS `workTime`,`t_user_work`.`workPlace` AS `workPlace`,`t_user_work`.`createAt` AS `createAt`,`t_user_work`.`reviewAt` AS `reviewAt`,`t_user_work`.`imagePath` AS `imagePath`,`t_work`.`workName` AS `workName`,`t_rank`.`rankName` AS `rankName`,`t_review`.`reviewName` AS `reviewName`,`v_teacher_info`.`username` AS `username`,`v_teacher_info`.`teaNo` AS `teaNo`,`v_teacher_info`.`name` AS `name`,`v_teacher_info`.`genderName` AS `genderName`,`t_user_work`.`userId` AS `userId`,`t_user_work`.`workId` AS `workId`,`t_user_work`.`reviewId` AS `reviewId`,`v_teacher_info`.`genderId` AS `genderId` from ((((`t_user_work` join `t_work` on((`t_user_work`.`workId` = `t_work`.`id`)))  join `t_review` on((`t_user_work`.`reviewId` = `t_review`.`id`))) join `v_teacher_info` on((`t_user_work`.`userId` = `v_teacher_info`.`userId`)));

-- ----------------------------
-- View structure for v_teacher_info
-- ----------------------------
DROP VIEW IF EXISTS `v_teacher_info`;
CREATE VIEW `v_teacher_info` AS select `t_user`.`id` AS `userId`,`t_user`.`username` AS `username`,`t_user`.`createAt` AS `createAt`,`t_user`.`updateAt` AS `updateAt`,`t_user_role`.`roleId` AS `roleId`,`t_role`.`roleName` AS `roleName`,`t_role`.`roleNameEn` AS `roleNameEn`,`t_teacher`.`teaNo` AS `teaNo`,`t_teacher`.`teaName` AS `name`,`t_gender`.`genderName` AS `genderName`,`t_teacher`.`genderId` AS `genderId` from ((((`t_user` join `t_user_role` on((`t_user_role`.`userId` = `t_user`.`id`))) join `t_role` on((`t_user_role`.`roleId` = `t_role`.`id`))) join `t_teacher` on((`t_user`.`username` = `t_teacher`.`teaNo`))) join `t_gender` on((`t_teacher`.`genderId` = `t_gender`.`id`)));
